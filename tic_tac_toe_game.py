...
def player_turn(board_list, current_player):  # Taking a turn
    ...
    while not player_chosen_a_square:  # Player choosing a square
        ...
        if (
            not player_square_choice.isdigit()
            or not 1 <= int(player_square_choice) <= 9
        ):
            print("\nInvalid input\n")
...
def check_for_winner(board_list):  # Checking if there's a winner
    for i in winning_combinations:
        if board_list[(i[0])] == board_list[(i[1])] == board_list[(i[2])] and (
            board_list[(i[0])] == "X" or board_list[(i[0])] == "O"
        ):
            if board_list[(i[0])] == "X":
